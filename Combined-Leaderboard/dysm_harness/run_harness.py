"""
Do-You-See-Me (perception) evaluation harness.

Reads the public sample set (tasks/do_you_see_me/questions.json), builds a
prompt for every sample, queries your model through a pluggable adapter, and
writes ONE submission file in the exact format the leaderboard expects:
...
Do-You-See-Me is a free-form perception task (no multiple-choice options), so
only the `standard` condition is produced.
...
root (the workspace folder that contains `Do-You-See-Me/`). Override with
...
# Combined-Leaderboard/dysm_harness/run_harness.py -> repo root is two levels up.
HARNESS_DIR = Path(__file__).resolve().parent
COMBINED_ROOT = HARNESS_DIR.parent
DEFAULT_QUESTIONS = COMBINED_ROOT / "tasks" / "do_you_see_me" / "questions.json"
...
def build_prompt(sample: dict, image_root: Path) -> dict:
    ...
    absolute path to the image file (or None if it could not be resolved).
    """
    image = None
...
def main():
    ap = argparse.ArgumentParser(description="Do-You-See-Me evaluation harness")
    ...
    ap.add_argument("--out", type=Path, default=Path("dysm_responses.json"))
    ap.add_argument("--image-root", type=Path, default=DEFAULT_IMAGE_ROOT,
                    help="folder containing the Do-You-See-Me/ dataset directory")
