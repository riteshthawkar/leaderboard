...
EXPECTED_CONTRACT="${EXPECTED_CONTRACT:-e00f947834d757085a921a84ce294d244ef51294ea57d0b7ad1772d9120ccedb}"
EXPECTED_CODE_BUNDLE="${EXPECTED_CODE_BUNDLE:-7a8ffdafafd4f632c7f94d9925500d444279e83629caf8fbd5c4090652249f8e}"
GPU_IDS="${GPU_IDS:-0,1}"
PORT="${PORT:-8075}"
...
code_bundle="$(
    sha256sum \
        evaluation/evidence_contract.py \
...
[[ "$code_bundle" == "$EXPECTED_CODE_BUNDLE" ]] || {
    log "Extractor code bundle mismatch: expected $EXPECTED_CODE_BUNDLE, found $code_bundle"
    exit 1
}
find "$SOURCE_ROOT" -type f -perm /222 -print -quit | grep -q . && {
    log "Canonical source tree must be read-only: $SOURCE_ROOT"
    exit 1
}
...
log "Starting pinned v5 extractor on GPUs $GPU_IDS for $candidate_count responses"
: >"$SERVER_LOG"
setsid env \
...
log "Canonical v5 evidence audit completed successfully"
